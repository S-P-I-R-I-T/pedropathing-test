...
@Autonomous
@Configurable // Panels
public class ANTRedFarA extends OpMode {
    ...
    private final Pose startPose = new Pose(86.5, 8, Math.toRadians(90)); //todo: 오차 심하면 0으로 바꾸기!!
    private final Pose shootPose = new Pose(86.5, 15, Math.toRadians(0));

    private final Pose wpPose = new Pose(106.5, 15, Math.toRadians(0));
    private final Pose gppPose = new Pose(129.5, 15, Math.toRadians(0));

    private final Pose leavePose = new Pose(93.5, 10, Math.toRadians(180));
    // A에선 전환시간후 imu기반 드라이빙 텔리를 해야하므로 시작위치와 가깝게 0도로 맞춤
